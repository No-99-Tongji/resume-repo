...
\centerline{\normalsize{wx：ll18228979484 \quad base：上海 \quad github : https://github.com/No-99-Tongji }}
% 把你github地址写上去  
% \centerline{\normalsize{可连续实习6个月以上 \quad 两周内到岗}}
\centerline{\normalsize{可连续实习6个月以上}}
...
\begin{itemize}
    ...
    \item \textbf{技术方案设计（与mentor共同讨论确定）：}
    \begin{itemize}
        \item \textbf{懒加载：} 下游请求触发更新，避免创建订单时的冗余API调用
        \item \textbf{分级缓存：} Redis（一级缓存）+ PostgreSQL（二级持久化），为下游提供高效查询服务
        \item \textbf{定期更新：} Quarkus定时任务每秒扫描需更新订单（比较Last\_pulled\_at字段），批量拉取最新状态
        \item \textbf{定点更新：} 下游检测缓存过期时，触发单个订单实时更新% 检测机制待完善
    \end{itemize}

    \item \textbf{技术实现细节：}
    \begin{itemize}
        \item \textbf{缓存过期检测：} % 方案1: 基于时间戳比较；方案2: 主动失效通知（由Claude生成，需确认）
        \item \textbf{Redis缓存策略：} 使用order\_id作为单key；各平台独立数据库与容器，确保数据隔离% 设计原因待向mentor确认
        \item \textbf{数据库设计：} 在seller\_id和last\_pulled\_at字段建立索引，优化批量查询性能
        \item \textbf{系统架构：} 采用Controller-Service-Fetcher-Repository分层架构
    ...
    \begin{itemize}
        ...
        \item \textbf{Redis：} 作为一级缓存，内存存储提供极高读写性能，满足订单状态查询的低延迟需求
        \item \textbf{PostgreSQL：} % 选型原因待向mentor确认（如：ACID特性强、JSON支持好等）
        \item \textbf{OpenAPI Generator：} 自动生成与各电商平台API对接的客户端代码，减少手动编写工作量，降低人为错误
        \item \textbf{架构设计：} Controller-Service-Fetcher-Repository分层架构，清晰分离关注点，便于维护与扩展
    \end{itemize}

    \item \textbf{量化成果与性能指标（待补充具体数字）：}
    \begin{itemize}
        \item API调用成本降低：[待补充]\%（相比直接调用上游API）
        \item Redis缓存命中率：[待补充]\%（一级缓存直接响应比例）
        \item 平均响应时间：[待补充]ms（缓存命中情况下）
        \item 系统吞吐量：[待补充]QPS / 日均处理订单数：[待补充]
        \item 数据一致性：缓存过期时间低于拉取间隔，确保读取不到过期数据
    \end{itemize}
    ...
    \item \textbf{技术栈：} Kotlin, Quarkus, Redis, PostgreSQL, OpenAPI Generator, 多平台电商API
\end{itemize}
...
\begin{itemize}
    ...
    \begin{itemize}
        ...
        \begin{itemize}
            \item \textbf{AI模型集成：} 调用OpenAI GPT-3.5 API实现自然语言处理% 可根据实际情况调整模型名称
            \item \textbf{知识检索：} 设计向量化检索系统，从课程、作业、通知等数据中提取相关上下文
            \item \textbf{Prompt工程：} 设计多层prompt模板，包含系统角色指令、检索上下文、对话历史、用户问题
            \item \textbf{上下文管理：} 实现对话状态维护，控制上下文窗口长度，保证多轮对话连贯性
            \item \textbf{错误处理：} 实现API调用重试、超时降级、异常回复等容错机制
            \item \textbf{性能优化：} 使用Redis缓存高频问题和标准答案，减少API调用成本与响应延迟
        \end{itemize}
    \end{itemize}
